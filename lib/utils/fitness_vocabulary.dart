/// 健身术语中英文对照表
/// Fitness Vocabulary - English to Chinese Translation
/// 
/// 这个文件包含所有健身相关术语的中英文对照
/// 用于翻译运动名称和健身术语

const Map<String, String> fitnessVocabulary = {
  // ==================== 器械 Equipment ====================
  'barbell': '杠铃',
  'dumbbell': '哑铃',
  'dumbbells': '哑铃',
  'cable': '绳索',
  'cables': '绳索',
  'machine': '器械',
  'kettlebell': '壶铃',
  'kettlebells': '壶铃',
  'kettle': '壶铃',
  'band': '弹力带',
  'bands': '弹力带',
  'medicine ball': '药球',
  'exercise ball': '健身球',
  'foam roll': '泡沫轴',
  'smith': '史密斯机',
  'ez': '曲杆',
  'ez-bar': '曲杆',
  'ez bar': '曲杆',
  'ez-barbell': '曲杆',
  
  // ==================== 动作类型 Exercise Types ====================
  'bench press': '卧推',
  'benchpress': '卧推',
  'squat': '深蹲',
  'deadlift': '硬拉',
  'row': '划船',
  'curl': '弯举',
  'press': '推举',
  'raise': '举',
  'raises': '举',
  'flyes': '飞鸟',
  'flye': '飞鸟',
  'fly': '飞鸟',
  'pull-up': '引体向上',
  'pullup': '引体向上',
  'pull up': '引体向上',
  'push-up': '俯卧撑',
  'pushup': '俯卧撑',
  'push up': '俯卧撑',
  'lunge': '弓步',
  'lunges': '弓步',
  'dip': '臂屈伸',
  'dips': '臂屈伸',
  'crunch': '卷腹',
  'crunches': '卷腹',
  'sit-up': '仰卧起坐',
  'situp': '仰卧起坐',
  'sit up': '仰卧起坐',
  'plank': '平板支撑',
  'extension': '伸展',
  'rollover': '翻滚',
  'rollout': '滚轮',
  'twist': '转体',
  'twists': '转体',
  'kickback': '后踢',
  'kickbacks': '后踢',
  'shrug': '耸肩',
  'shrugs': '耸肩',
  'pullover': '仰卧臂屈伸',
  'pulldown': '下拉',
  'pull down': '下拉',
  'pushdown': '下压',
  'push down': '下压',
  'clean': '翻举',
  'snatch': '抓举',
  'jerk': '挺举',
  'thrust': '推举',
  'thrusts': '推举',
  'bridge': '桥式',
  'glute bridge': '臀桥',
  'hip thrust': '臀推',
  'hip raise': '臀桥',
  'good morning': '早安式',
  'hack squat': '哈克深蹲',
  'leg press': '腿举',
  'leg curl': '腿弯举',
  'leg extension': '腿屈伸',
  'calf raise': '提踵',
  'calf raises': '提踵',
  'clean and jerk': '翻举与挺举',
  'clean & jerk': '翻举与挺举',
  'clean and press': '翻举与推举',
  'power clean': '爆发力翻举',
  'power snatch': '爆发力抓举',
  'sumo': '相扑',
  
  // ==================== 身体部位 Body Parts ====================
  'chest': '胸',
  'back': '背',
  'shoulder': '肩',
  'shoulders': '肩部',
  'leg': '腿',
  'legs': '腿部',
  'arm': '臂',
  'arms': '手臂',
  'bicep': '二头肌',
  'biceps': '二头肌',
  'tricep': '三头肌',
  'triceps': '三头肌',
  'abs': '腹肌',
  'abdominals': '腹肌',
  'ab': '腹',
  'core': '核心',
  'glute': '臀',
  'glutes': '臀部',
  'quad': '四头肌',
  'quads': '股四头肌',
  'quadriceps': '股四头肌',
  'hamstring': '腘绳肌',
  'hamstrings': '腘绳肌',
  'calf': '小腿',
  'calves': '小腿',
  'forearm': '前臂',
  'forearms': '前臂',
  'lat': '背阔肌',
  'lats': '背阔肌',
  'latissimus dorsi': '背阔肌',
  'trap': '斜方肌',
  'traps': '斜方肌',
  'trapezius': '斜方肌',
  'delt': '三角肌',
  'delts': '三角肌',
  'deltoid': '三角肌',
  'oblique': '腹斜肌',
  'obliques': '腹斜肌',
  'lower back': '下背部',
  'middle back': '中背部',
  'upper back': '上背部',
  
  // ==================== 方向/位置 Direction/Position ====================
  'incline': '上斜',
  'decline': '下斜',
  'flat': '平躺',
  'seated': '坐姿',
  'standing': '站姿',
  'lying': '仰卧',
  'prone': '俯卧',
  'supine': '仰卧',
  'one-arm': '单臂',
  'one arm': '单臂',
  'two-arm': '双臂',
  'two arm': '双臂',
  'single-arm': '单臂',
  'single leg': '单腿',
  'single-leg': '单腿',
  'alternate': '交替',
  'alternating': '交替',
  'close-grip': '窄握',
  'close grip': '窄握',
  'wide-grip': '宽握',
  'wide grip': '宽握',
  'medium grip': '中握',
  'neutral grip': '对握',
  'reverse grip': '反握',
  'overhand': '正握',
  'underhand': '反握',
  'hammer': '锤式',
  'front': '前',
  'rear': '后',
  'side': '侧',
  'lateral': '侧向',
  'overhead': '过顶',
  'behind neck': '颈后',
  'behind back': '背后',
  
  // ==================== 特殊动作名称 Special Exercise Names ====================
  'arnold': '阿诺德',
  'preacher': '牧师凳',
  'concentration': '集中',
  'scaption': '肩胛平面举',
  'upright': '直立',
  'renegade': '叛徒式',
  'bulgarian': '保加利亚',
  'russian': '俄罗斯',
  'turkish': '土耳其',
  'zottman': 'Zottman',
  'zott': 'Zott',
  'skull crusher': '碎颅者',
  'skullcrusher': '碎颅者',
  'face pull': '面拉',
  'landmine': '地雷管',
  'prowler': '推雪橇',
  'sled': '雪橇',
  'sledgehammer': '大锤',
  'farmer': '农夫',
  'bear': '熊',
  'frog': '青蛙',
  'spider': '蜘蛛',
  'dead bug': '死虫',
  'bird dog': '鸟狗',
  'superman': '超人',
  'burpee': '波比跳',
  'kipping': '摆浪',
  'rope': '爬绳',
  'battle': '战绳',
  
  // ==================== 难度/类型 Difficulty/Type ====================
  'beginner': '初级',
  'intermediate': '中级',
  'advanced': '高级',
  'expert': '专家级',
  'compound': '复合',
  'isolation': '孤立',
  'bodyweight': '自重',
  'body only': '自重',
  'body weight': '自重',
  'explosive': '爆发',
  'plyo': '增强式',
  'plyometric': '增强式',
  'assisted': '辅助',
  'weighted': '负重',
  
  // ==================== 常见词 Common Words ====================
  'circles': '画圈',
  'stretches': '拉伸',
  'stretch': '拉伸',
  'holds': '保持',
  'jumps': '跳跃',
  'sprints': '冲刺',
  'drags': '拖拽',
  'throws': '投掷',
  'rolls': '滚',
  'hops': '跳',
  'climb': '爬',
  'crawl': '爬行',
  
  // ==================== 器械和位置补充 Equipment & Position Extras ====================
  'bench': '长凳',
  'board': '板',
  'floor': '地板',
  'roller': '滚轮',
  'bosu ball': '波速球',
  'bosu': '波速',
  'ball': '球',
  'atlas': '阿特拉斯',
  'stone': '石',
  'stones': '石',
  'axle': '轴杆',
  'bar': '杆',
  'pulley': '滑轮',
  'chain': '链条',
  'chains': '链条',
  'weight': '重量',
  'weights': '重量',
  
  // ==================== 身体部位补充 Body Parts Extras ====================
  'ankle': '脚踝',
  'heel': '脚跟',
  'knee': '膝盖',
  'knees': '膝盖',
  'hip': '髋',
  'hips': '髋部',
  'pelvis': '骨盆',
  'spine': '脊柱',
  'neck': '颈部',
  'head': '头',
  'face': '脸',
  'hand': '手',
  'hands': '手',
  'foot': '脚',
  'feet': '脚',
  'toe': '脚趾',
  'toes': '脚趾',
  'finger': '手指',
  'fingers': '手指',
  'thumb': '拇指',
  'wrist': '手腕',
  'elbow': '肘部',
  'elbows': '肘部',
  'groin': '腹股沟',
  'tibialis': '胫骨前肌',
  'anterior': '前侧',
  
  // ==================== 方向和位置补充 Direction/Position Extras ====================
  'bent': '弯曲',
  'hang': '悬挂',
  'hanging': '悬垂',
  'mid': '中部',
  'full': '完整',
  'diagonal': '对角线',
  'backward': '向后',
  'forward': '向前',
  'upward': '向上',
  'downward': '向下',
  'inward': '向内',
  'outward': '向外',
  'horizontal': '水平',
  'vertical': '垂直',
  'parallel': '平行',
  'perpendicular': '垂直',
  'bottoms-up': '倒置',
  'bottoms up': '倒置',
  'guillotine': '断头台',
  'anti-gravity': '反重力',
  
  // ==================== 动作描述补充 Action Descriptions ====================
  'air': '空气',
  'air bike': '空中蹬车',
  'bike': '自行车',
  'bicycling': '骑自行车',
  'stationary': '固定',
  'touchers': '触碰',
  'through': '通过',
  'apart': '分开',
  'all fours': '四肢着地',

  'palms': '手掌',
  'palms in': '掌心向内',
  'palms up': '掌心向上',
  'palms down': '掌心向下',
  'sprint': '冲刺',
  'bound': '跳跃',
  'bounds': '跳跃',
  'body-up': '起身',
  'body up': '起身',
  'battling': '战斗',
  'wide stance': '宽站距',
  'stance': '站距',
  'stiff': '直腿',
  'wind': '风速',
  
  // ==================== 其他常用动作词汇 Other Common Terms ====================
  'push': '推',
  'pull': '拉',
  'jump': '跳',
  'kick': '踢',
  'punch': '拳击',
  'box': '拳击',
  'step': '踏步',
  'mountain': '山地',
  'mountain climber': '登山者',
  'russian twist': '俄式转体',
  'turkish get-up': '土耳其起身',
  'farmer\'s walk': '农夫行走',
  'bear crawl': '熊爬',
  'spider crawl': '蜘蛛爬',
  'frog jump': '青蛙跳',
  'box jump': '跳箱',
  'jump squat': '跳蹲',
  'jumping jack': '开合跳',
  'high knees': '高抬腿',
  'butt kick': '后踢腿',
  'butt kicks': '后踢腿',
  'mountain climbers': '登山者',
  'battle rope': '战绳',
  'rope climb': '爬绳',
  'sled push': '推雪橇',
  'sled pull': '拉雪橇',
  'prowler push': '推雪橇',
  'medicine ball slam': '药球砸地',
  'ball slam': '砸球',
  'slam': '砸',
  'wall ball': '墙球',
  'wall sit': '靠墙静蹲',
  'box step-up': '踏箱',
  'step-up': '上步',
  'step up': '上步',
  'step-down': '下步',
  'step down': '下步',
  'split squat': '分腿蹲',
  'split': '分腿',
  'sumo squat': '相扑深蹲',
  'sumo deadlift': '相扑硬拉',
  'goblet squat': '高脚杯深蹲',
  'goblet': '高脚杯',
  'front squat': '前蹲',
  'back squat': '后蹲',
  'overhead squat': '过顶深蹲',
  'front raise': '前平举',
  'side raise': '侧平举',
  'lateral raise': '侧平举',
  'rear delt raise': '后三角肌举',
  'rear delt fly': '后三角肌飞鸟',
  'upright row': '直立划船',
  'barbell shrug': '杠铃耸肩',
  'dumbbell shrug': '哑铃耸肩',
  'preacher curl': '牧师凳弯举',
  'concentration curl': '集中弯举',
  'hammer curl': '锤式弯举',
  'bicep curl': '二头肌弯举',
  'tricep extension': '三头肌伸展',
  'tricep pushdown': '三头肌下压',
  'tricep dip': '三头肌臂屈伸',
  'chest fly': '胸部飞鸟',
  'chest press': '胸推',
  'incline press': '上斜推',
  'decline press': '下斜推',
  'incline bench': '上斜卧推',
  'decline bench': '下斜卧推',
  'flat bench': '平板卧推',
  'barbell row': '杠铃划船',
  'dumbbell row': '哑铃划船',
  'cable row': '绳索划船',
  'lat pulldown': '背阔肌下拉',
  'chin-up': '反握引体向上',
  'chin up': '反握引体向上',
  'romanian deadlift': '罗马尼亚硬拉',
  'rdl': '罗马尼亚硬拉',
  'stiff-leg deadlift': '直腿硬拉',
  'stiff leg deadlift': '直腿硬拉',
  'seated calf raise': '坐姿提踵',
  'standing calf raise': '站姿提踵',
  'donkey calf raise': '驴式提踵',
  'sissy squat': '西斯深蹲',
  'walking lunge': '行走弓步',
  'reverse lunge': '后退弓步',
  'side lunge': '侧弓步',
  'curtsy lunge': '屈膝礼弓步',
  'bulgarian split squat': '保加利亚分腿蹲',
  'hip abduction': '髋外展',
  'hip adduction': '髋内收',
  'abductor': '外展肌',
  'adductor': '内收肌',
  'hyperextension': '背部伸展',
  'back extension': '背部伸展',
  'roman chair': '罗马椅',
  'leg raise': '举腿',
  'hanging leg raise': '悬垂举腿',
  'knee raise': '屈膝举腿',
  'hanging knee raise': '悬垂屈膝举腿',
  'side plank': '侧平板支撑',
  'bicycle crunch': '自行车卷腹',
  'bicycle': '自行车',
  'tuck jump': '屈膝跳',
  'broad jump': '立定跳远',
  'long jump': '跳远',
  'vertical jump': '垂直跳',
  'rope slam': '战绳砸',
  'rope wave': '战绳波浪',
  'farmer\'s carry': '农夫提走',
  'farmer carry': '农夫提走',
  'suitcase carry': '手提箱提走',
  'rack carry': '架上提走',
  'overhead carry': '过顶提走',
  'waiter\'s carry': '服务员提走',
  'waiter carry': '服务员提走',
  'zercher carry': '泽奇提走',
  'windmill': '风车',
  'halo': '光环',
  'around the world': '环绕世界',
  'iron cross': '铁十字',
  'flag': '旗',
  'human flag': '人体旗帜',
  'muscle-up': '双力臂',
  'muscle up': '双力臂',
  'pistol squat': '手枪深蹲',
  'pistol': '手枪',
  'shrimp squat': '虾深蹲',
  'cossack squat': '哥萨克深蹲',
  'dragon pistol': '龙式手枪深蹲',
  'planche': '十字支撑',
  'front lever': '前水平',
  'back lever': '后水平',
  'lever': '水平',
  'handstand': '倒立',
  'handstand push-up': '倒立撑',
  'handstand pushup': '倒立撑',
  'cartwheel': '侧手翻',
  'roundoff': '侧手翻内转',
  'backflip': '后空翻',
  'front flip': '前空翻',
  'flip': '空翻',
  'vault': '跳马',
  'kong vault': '猩猩跳',
  'precision jump': '精准跳',
  'cat leap': '猫扑',
  'wall run': '跑墙',
  'tic tac': '踩墙',
  'balance': '平衡',
  'equilibrium': '平衡',
  'stability': '稳定',
  'mobility': '灵活性',
  'flexibility': '柔韧性',
  'agility': '敏捷性',
  'coordination': '协调性',
  'endurance': '耐力',
  'strength': '力量',
  'power': '爆发力',
  'speed': '速度',
  'cardio': '有氧',
  'aerobic': '有氧',
  'anaerobic': '无氧',
  'hiit': '高强度间歇',
  'tabata': '塔巴塔',
  'circuit': '循环训练',
  'crossfit': '交叉训练',
  'olympic': '奥林匹克',
  'weightlifting': '举重',
  'powerlifting': '力量举',
  'strongman': '壮汉',
  'bodybuilding': '健美',
  'calisthenics': '徒手健身',
  'gymnastics': '体操',
  'yoga': '瑜伽',
  'pilates': '普拉提',
  'stretching': '拉伸',
  'foam rolling': '滚泡沫轴',
  'massage': '按摩',
  'recovery': '恢复',
  'warm-up': '热身',
  'warm up': '热身',
  'cooldown': '冷身',
  'cool down': '冷身',
  'rest': '休息',
  'set': '组',
  'rep': '次',
  'repetition': '次数',
  'reps': '次数',
  'volume': '容量',
  'intensity': '强度',
  'frequency': '频率',
  'duration': '持续时间',
  'tempo': '节奏',
  'rhythm': '韵律',
  'form': '动作形式',
  'technique': '技术',
  'posture': '姿势',
  'alignment': '对齐',
  'breathing': '呼吸',
  'inhale': '吸气',
  'exhale': '呼气',
  'contraction': '收缩',
  'eccentric': '离心',
  'concentric': '向心',
  'isometric': '等长',
  'isotonic': '等张',
  'peak': '顶峰',
  'squeeze': '挤压',
  'hold': '保持',
  'pause': '暂停',
  'lockout': '锁定',
  'failure': '力竭',
  'pump': '充血',
  'burn': '燃烧',
  'fatigue': '疲劳',
  'soreness': '酸痛',
  'doms': '延迟性肌肉酸痛',
  'injury': '受伤',
  'pain': '疼痛',
  'discomfort': '不适',
  'strain': '拉伤',
  'sprain': '扭伤',
  'inflammation': '炎症',
  'swelling': '肿胀',
  'bruise': '淤青',
  'cramp': '抽筋',
  'spasm': '痉挛',
  'stiffness': '僵硬',
  'tightness': '紧张',
  'tension': '张力',
  'release': '释放',
  'relax': '放松',
  'loosen': '放松',
  'mobilize': '活动化',
  'activate': '激活',
  'stabilize': '稳定',
  'engage': '参与',
  'contract': '收缩',
  'extend': '伸展',
  'flex': '弯曲',
  'rotate': '旋转',
  'bend': '弯曲',
  'straighten': '伸直',
  'lift': '举起',
  'lower': '放下',
  'drop': '下降',
  'abduct': '外展',
  'adduct': '内收',
  'hyperextend': '过伸',
  'circumduct': '环转',
  'tilt': '倾斜',
  'shift': '移位',
  'hinge': '铰链',
  'fold': '折叠',
  'unfold': '展开',
  'open': '打开',
  'close': '关闭',
  'spread': '展开',
  'pinch': '捏',
  'grip': '握',
  'grab': '抓',
  'throw': '投',
  'catch': '接',
  'bounce': '弹',
  'slide': '滑',
  'swing': '摆动',
  'rock': '摇晃',
  'shake': '抖动',
  'tap': '轻拍',
  'slap': '拍打',
  'strike': '击打',
  'stomp': '跺脚',
  'march': '行进',
  'walk': '走',
  'run': '跑',
  'jog': '慢跑',
  'skip': '跳步',
  'leap': '跃',

  'spring': '弹跳',
  'turn': '转身',
  'spin': '旋转',
  'pivot': '枢轴旋转',
  'circle': '画圈',
  'arc': '弧线',
  'wave': '波浪',
  'snake': '蛇形',
  'spiral': '螺旋',
  'figure-8': '8字形',
  'infinity': '无穷大形',
};

/// 翻译运动名称
/// 将英文运动名称翻译成中文
String translateExerciseName(String englishName) {
  if (englishName.isEmpty) return englishName;
  
  String result = englishName;
  final lowerName = englishName.toLowerCase();
  
  // 按长度排序，优先匹配更长的词组
  final sortedTerms = fitnessVocabulary.keys.toList()
    ..sort((a, b) => b.length.compareTo(a.length));
  
  // 逐个替换词汇
  for (final term in sortedTerms) {
    if (lowerName.contains(term)) {
      result = result.replaceAll(
        RegExp(RegExp.escape(term), caseSensitive: false),
        fitnessVocabulary[term]!,
      );
    }
  }
  
  // 清理多余空格和标点
  result = result
      .replaceAll(RegExp(r'\s+'), ' ')
      .replaceAll(RegExp(r'-+'), '-')
      .replaceAll(RegExp(r'\s*-\s*'), '-')
      .trim();
  
  // 移除中文后的复数s
  result = result.replaceAllMapped(
    RegExp(r'([\u4e00-\u9fa5])s(\s|,|\-|$)'),
    (match) => '${match.group(1)}${match.group(2)}',
  );
  
  return result;
}
